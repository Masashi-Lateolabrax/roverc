...
> **注記 (2026-06-09)**: 実験プラットフォームは 2026-06-09 に **前方単眼1台 (no fisheye)** へ変更。Timer Camera F（魚眼）は現行リグ非搭載・在庫温存となり、本ロードマップは将来ライン。再開には魚眼カメラの追加搭載が必要。なお calibration_thesis の単眼化後 ground truth は前方単眼 + マーカー方式に変更済み（魚眼 VIO は不使用）。
